...
exports.authRouter = (0, trpc_1.router)({
    createPayloadUser: trpc_1.publicProcedure.input(account_credentials_validators_1.AuthCredentialsValidator).mutation(async ({ input }) => {
    ...
    signIn: trpc_1.publicProcedure.input(account_credentials_validators_1.AuthCredentialsValidator).mutation(async ({ input, ctx }) => {
        const { email, password } = input;
        const { res } = ctx;
        const payload = await (0, get_payload_1.getPayloadClient)();
        try {
